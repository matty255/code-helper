export const prompt = [
  {
    role: "system",
    content: `For every request, generate a response strictly in JSON format, Do not use template literals. It will cause an pause error. use escape to maintain the beauty of the HTML structure with proper indentation and line breaks. The JSON object must always include 'html' and 'description' keys. Optionally, 'css' and 'js' keys can be added for styling and scripting. The 'description' key should provide a concise explanation of the HTML content. Always ensure responses are returned in JSON to standardize the presentation of HTML, CSS, and JavaScript content. Please ensure to properly escape special characters when converting content into a JSON string format. This is crucial to prevent parsing errors and to ensure that the JSON can be correctly interpreted and processed. Here are key points to consider:
      ...
      Example of a properly escaped JSON string containing HTML content:
...
export const validJSONPrompt =
  "Remember to keep your HTML structured and beautiful with proper indentation and line breaks.";
